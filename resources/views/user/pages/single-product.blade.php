                            ...
                            <div class="col-md-12 col-12">
                                ...
                                <div class="mb-3">
                                    ...
                                    @if($product->type == 'single')
                                        ...
                                        @endif
                                    @else
                                        <span class="single-product-bg-info" id="product_price_info{{optional($product)->id}}">Price Range: {{number_format($min_price)}}৳ <span class="price__divided"></span> {{number_format($max_price)}}৳ </span> 
                                    @endif 
                                    {{-- Status --}}
                                    <span class="single-product-bg-info">Status: {{$stock_qty_text}}</span>
                                    {{-- Producr Code --}}
                                    <span class="single-product-bg-info">Producr Code: {{ $product->code }}</span>
                                </div>
                            </div>
                        </div>                        
                        ...
                        <div class="product__variant">
                            ...
                            <div>
                                ...
                                @if($product->type == 'single')
                                    ...
                                    @if(optional($stock_price)->qty > 0)
                                        <button class="ms-0 quickview__cart--btn primary__btn" onclick="addToCart({{optional($product)->id}}, 'details', 'cart', 'single')" id="add_to_cart_button{{optional($product)->id}}" type="button">Add To Cart</button>  
                                        <button class="quickview__cart--btn primary__btn" id="buy_now_button{{optional($product)->id}}" onclick="addToCart({{optional($product)->id}}, 'details', 'checkout', 'single')" type="button">Buy Now</button>
                                    @endif  
                                @else
                                    <input type="hidden" name="product_type" id="product_type" value="variation">
                                    <input type="hidden" name="stock_qty" id="stock_qty_{{optional($product)->id}}" value="0">
                                    <button class="ms-0 quickview__cart--btn primary__btn" id="add_to_cart_button{{optional($product)->id}}" onclick="addToCart({{optional($product)->id}}, 'details', 'cart', 'variation')" type="button">Add To Cart</button>  
                                    <button class="quickview__cart--btn primary__btn mx-3" id="buy_now_button{{optional($product)->id}}" onclick="addToCart({{optional($product)->id}}, 'details', 'checkout', 'variation')" type="button">Buy Now</button>  
